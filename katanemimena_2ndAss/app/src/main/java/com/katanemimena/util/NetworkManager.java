...
public class NetworkManager {
    public static final String SYSTEM_CONTROLLER_IP = "192.168.1.2";
    public static final int SYSTEM_CONTROLLER_PORT = 778;
    ...
    public LiveData<List<JSONObject>> callSystemControllerGetInformation(){ //kalei 1h fora gia na parei thn plhroforia
    ...
    public LiveData<List<JSONObject>> callSearchForMusic(final String artist){ //kalei gia na anazitisei ta tragoudia tou artist
        final MutableLiveData<List<JSONObject>> result = new MutableLiveData<>();
        new Thread( new Runnable() {
            @Override
            public void run() {
    ...
    public LiveData<DataHolder> callDownloadMusic(final String artist, final String song){ //download song
        ...
        new Thread( new Runnable() {
            ...
            public void run() {
                // IN BACKGROUND THREAD to avoid blocking the UI
                try{
                    Pair<String, Integer> brokerDetailsForArtist = getBrokerDetailsForArtist(artist); //string and integer

                    Socket socket = new Socket(brokerDetailsForArtist.first, brokerDetailsForArtist.second);
                    PrintWriter pr = new PrintWriter(socket.getOutputStream(), true);
                    BufferedReader br = new BufferedReader(new InputStreamReader(socket.getInputStream()));
                    pr.println(RequestBuilder.createGetMusicRequest(artist, song)); //grafei ston server
                    String response = br.readLine();
                    JSONObject resp = new JSONObject(response); //metatropi se json

                    //READING BYTES
                    int size_in_bytes = resp.getInt("size_in_bytes");
                    DataInputStream dis = new DataInputStream(socket.getInputStream());
                    byte[] songData = new byte[size_in_bytes];
                    dis.readFully(songData); //diavazei ola ta bytes

                    DataHolder dataHolder = new DataHolder(); //antikeimeno pou apothikevei bytes oloklirou tou tragoudiou
                    dataHolder.data = songData;
                    result.postValue(dataHolder); //allo thread uithread apo background thread
                }catch (Exception e){
    ...
    private Pair<String, Integer> getBrokerDetailsForArtist(String artist){
        String cachedStringFromSystemController = localStorage.getString(LocalStorage.CACHE_SYSTEM_CONTROLLER_RESPONSE); //epistrefei th cacharismeni apantisi pou gyrise o system controller the 1st time
        try{
